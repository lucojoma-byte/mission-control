...
class VercelBlobClaimStore {
  constructor({ putFn, prefix = 'guardian-lead-claims/v1' } = {}) {
    this.putFn = putFn || require('@vercel/blob').put;
    this.prefix = String(prefix).replace(/^\/+|\/+$/g, '');
  }

  pathname(leadId) {
    const normalized = normalizarLeadId(leadId);
    if (!normalized) throw new Error('leadId obligatorio para reclamo Blob');
    return `${this.prefix}/${normalized}.json`;
  }

  async claim(lead) {
    const leadId = normalizarLeadId(lead?.leadId);
    const pathname = this.pathname(leadId);
    try {
      await this.putFn(pathname, JSON.stringify({
        schema: 'guardian.lead.claim.v1',
        leadId,
      ...
      if (String(error?.message || '').includes('Vercel Blob: This blob already exists')) {
        return { acquired: false, state: 'already_claimed' };
      }
      throw error;
    }
  }

  async commit(leadIdInput) {
    const leadId = normalizarLeadId(leadIdInput);
    await this.putFn(this.pathname(leadId), JSON.stringify({
      schema: 'guardian.lead.claim.v1',
      leadId,
      state: 'committed',
    }), {
